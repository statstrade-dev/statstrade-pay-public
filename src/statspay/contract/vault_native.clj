(ns statspay.contract.vault-native
...
(defn.sol ^{:- [:internal :view]
            :static/returns [statspay.contract.vault-native/VaultAccount :memory]}
  ut:get-account
...
(defn.sol ^{:- [:internal :view]
            :static/returns [statspay.contract.vault-native/WithdrawRequest :memory]}
  ut:get-withdraw
...
(defn.sol ^{:- [:internal]}
  ...
  (cond arbitrated
        (. (s/payable -/g:SiteAuthority)
           (transfer vault-tax))
        
        :else
        (. (s/payable (. -/g:VaultLookup [vault-id] owner))
           (transfer vault-tax)))
  
  (. (s/payable user-address)
...
(defn.sol ^{:- [:external
  ...
  (s/require (== (. account status)
                 (. -/VaultAccountStatus Active))
             "Account not active.")
  (:+=  (. -/g:VaultLookup [vault-id] total-pool)
        s/msg-value)
...
(def +default-contract+
  {:ns   (h/ns-sym)
   :name "StatstradeVaultNativeFactory"
   :args [30]})
